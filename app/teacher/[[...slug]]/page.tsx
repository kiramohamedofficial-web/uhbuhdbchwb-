...
interface AppearanceContextType {
    ...
    profileHeaderColor: string; setProfileHeaderColor: (color: string) => void;
}
...
function TeacherPageInner() {
    ...
    useEffect(() => { localStorage.setItem(STORAGE_KEY, '/teacher'); }, []);
    ...
    useEffect(() => {
        const handleResume = async () => {
            ...
            if (isRefreshPaused || currentUser?.role === Role.SUPERVISOR) return;
    ...
    useEffect(() => {
        if (isRefreshPaused || !currentUser || currentUser.role === Role.SUPERVISOR) return;
    ...
    if (currentUser.role !== Role.TEACHER && currentUser.role !== Role.SUPERVISOR) { router.replace('/'); return null; }
...
export default function TeacherPage() {
    return <AppearanceProvider><TeacherPageInner /></AppearanceProvider>;
}
