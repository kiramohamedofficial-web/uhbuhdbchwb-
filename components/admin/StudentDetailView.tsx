...
const DetailRow: React.FC<{ label: string; value: string; icon?: any; isCopyable?: boolean }> = ({ label, value, icon: Icon, isCopyable }) => {
    ...
    return (
        <div className="flex items-center justify-between p-4 bg-[var(--bg-tertiary)]/50 rounded-2xl border border-[var(--border-primary)] group hover:border-[var(--accent-primary)]/30 transition-colors">
            ...
            {isCopyable && value && (
                <button onClick={handleCopy} className="p-2 text-[var(--text-secondary)] hover:text-[var(--accent-primary)] hover:bg-[var(--bg-secondary)] rounded-xl transition-all opacity-0 group-hover:opacity-100">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select value={plan} onChange={(e) => setPlan(e.target.value as any)} className="w-full p-3 rounded-xl bg-[var(--bg-tertiary)] border border-[var(--border-primary)] outline-none focus:border-[var(--accent-primary)] font-bold text-sm">
                    ...
                    <div>
                        ...
                        <select value={status} onChange={(e) => setStatus(e.target.value as any)} className="w-full p-3 rounded-xl bg-[var(--bg-tertiary)] border border-[var(--border-primary)] outline-none focus:border-[var(--accent-primary)] font-bold text-sm">
                 ...
                 <div>
                    ...
                    <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full p-3 rounded-xl bg-[var(--bg-tertiary)] border border-[var(--border-primary)] outline-none focus:border-[var(--accent-primary)] font-bold text-sm" />
                </div>
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="تعديل الملف الشخصي">
            <div className="space-y-4">
                ...
                <select value={formData.grade || ''} onChange={e => setFormData({...formData, grade: Number(e.target.value)})} className="w-full p-3 bg-[var(--bg-tertiary)] rounded-xl border border-[var(--border-primary)] font-bold outline-none">
    ...
    <div className="fade-in space-y-6 pb-20">
        ...
        <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
                <button onClick={() => setIsEditModalOpen(true)} className="p-3 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-xl text-[var(--text-secondary)] hover:text-indigo-600 shadow-sm transition-all"><PencilIcon className="w-5 h-5"/></button>
                <button onClick={() => { setIsPasswordModalOpen(true); setNewPassword(''); }} className="p-3 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-xl text-[var(--text-secondary)] hover:text-amber-500 shadow-sm transition-all"><KeyIcon className="w-5 h-5"/></button>
                <button onClick={() => setIsDeleteModalOpen(true)} className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-500 hover:bg-red-500 hover:text-white shadow-sm transition-all"><TrashIcon className="w-5 h-5"/></button>
            </div>
        </div>
                                ...
                                {activeSubscriptions.length > 0 ? activeSubscriptions.map((sub, idx) => (
                                    <div key={idx} className="p-5 rounded-[2rem] border-2 bg-emerald-500/5 border-emerald-500/20 transition-all">
                                        ...
                                        <div className="flex justify-between items-end">
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => { setSelectedSub(sub); setIsSubModalOpen(true); }} className="p-2 bg-white dark:bg-black/20 rounded-lg text-[var(--text-secondary)] hover:text-indigo-600 transition-colors shadow-sm"><PencilIcon className="w-4 h-4"/></button>
                                                <button onClick={() => handleCancelSub(sub)} className="p-2 bg-white dark:bg-black/20 rounded-lg text-[var(--text-secondary)] hover:text-red-600 transition-colors shadow-sm"><XCircleIcon className="w-4 h-4"/></button>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-right text-sm">
                                    <thead className="bg-[var(--bg-tertiary)] text-[var(--text-secondary)]">
                                        <tr>
                                            <th className="px-4 py-3 rounded-r-xl">الباقة</th>
                                            <th className="px-4 py-3">تاريخ البدء</th>
                                            <th className="px-4 py-3">تاريخ الانتهاء</th>
                                            <th className="px-4 py-3 rounded-l-xl text-center">الحالة</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-[var(--border-primary)]">
                                        {expiredSubscriptions.map((sub, idx) => (
                                            <tr key={idx} className="hover:bg-[var(--bg-tertiary)]/50 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-[var(--text-secondary)] text-sm">{new Date(sub.startDate).toLocaleDateString('ar-EG')}</td>
                                                <td className="px-4 py-3 text-[var(--text-secondary)] text-sm">{new Date(sub.endDate).toLocaleDateString('ar-EG')}</td>
                                        ...
                                        {expiredSubscriptions.length === 0 && (
                                            <tr>
                                                <td colSpan={4} className="px-4 py-8 text-center text-[var(--text-secondary)] italic opacity-60">السجل فارغ</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
