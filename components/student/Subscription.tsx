    ...
    return (
        <div className="max-w-4xl mx-auto pb-40 pt-10 px-4">
            ...
            <div className="text-center mb-16 reveal-item">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-indigo-500/10 text-indigo-600 text-sm font-black uppercase tracking-[0.2em] mb-4 border border-indigo-500/20">
                    Premium Access
                </div>
                <h1 className="text-4xl md:text-6xl font-black text-[var(--text-primary)] mb-4 tracking-tighter leading-tight">
                    استعد للـ <span className="text-indigo-600">تفوق</span> 🚀
                </h1>
            ...
            <div className="relative overflow-hidden rounded-[3rem] p-1 bg-gradient-to-br from-indigo-500 via-purple-600 to-pink-500 shadow-2xl reveal-item" style={{ animationDelay: '400ms' }}>
                <div className="bg-[var(--bg-secondary)] dark:bg-slate-900 rounded-[2.8rem] p-8 md:p-12 relative overflow-hidden">
                    {/* Decorative Background */}
                    <div className="absolute top-[-20%] right-[-10%] w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none"></div>
                    <div className="absolute bottom-[-20%] left-[-10%] w-64 h-64 bg-purple-500/10 rounded-full blur-3xl pointer-events-none"></div>

                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                        <div className="md:w-1/2 text-center md:text-right">
                            <div className="w-14 h-14 rounded-2xl bg-indigo-500/10 flex items-center justify-center text-indigo-600 mb-4 mx-auto md:mx-0">
                        ...
                        <form onSubmit={handleRedeemCode} className="md:w-1/2 w-full space-y-4">
                            <div className="relative group">
                                <input 
                                    ...
                                    className="w-full bg-[var(--bg-tertiary)] border-2 border-[var(--border-primary)] focus:border-indigo-500 rounded-3xl py-5 px-6 text-center font-mono text-2xl font-black tracking-[0.2em] outline-none transition-all placeholder:tracking-normal placeholder:font-sans placeholder:text-base placeholder:opacity-60 shadow-inner"
                                    dir="ltr"
                                    maxLength={20}
                                />
                                <div className="absolute inset-0 rounded-3xl border-2 border-indigo-500 opacity-0 group-focus-within:opacity-10 pointer-events-none transition-opacity"></div>
                            </div>
                            <button 
                                type="submit" 
                                disabled={isRedeeming || code.length < 4}
                                className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-3xl font-black text-lg shadow-xl shadow-indigo-500/30 transition-all transform active:scale-95 disabled:opacity-50 disabled:active:scale-100 flex items-center justify-center gap-3"
                            >
                                {isRedeeming ? (
