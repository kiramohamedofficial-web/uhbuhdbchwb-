...
const TeacherLayout: React.FC<TeacherLayoutProps> = ({ user, teacher, onLogout, children, onNavClick, activeView, supervisedTeachers, selectedTeacherId, onSelectTeacher }) => {
    ...
    return (
        <div className="h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] p-0 md:p-4 overflow-hidden flex flex-col md:grid md:grid-rows-1 gap-0"
            style={{ gridTemplateColumns: isSidebarCollapsed ? '80px 1fr' : '256px 1fr' }}>

            {/* Desktop Sidebar - Fixed Z-index and Toggle position */}
            <aside className={`flex-shrink-0 hidden md:flex flex-col rounded-[2rem] z-40 relative bg-[var(--bg-secondary)] border border-[var(--border-primary)] shadow-2xl transition-all duration-500 m-2 mr-0`}>
