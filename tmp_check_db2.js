...
async function testQuery() {
    console.log("Testing exact initData query...");
    const { data: grades, error } = await supabase.from('grades').select('*, semesters(*, units(*, lessons(*)))');
    if (error) {
        console.error("Error:", error.message, error.details, error.hint);
    } else {
        console.log("Query returned successfully. Grades length:", grades?.length);
    }
}
